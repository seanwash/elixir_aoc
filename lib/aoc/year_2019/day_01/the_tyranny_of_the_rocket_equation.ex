defmodule Aoc.Year2019.Day01.TheTyrannyoftheRocketEquation do
  @moduledoc """
  ...
  *What is the sum of the fuel requirements* for all of the modules on your
  ...
  def part_2(input) do
    input
    |> process_input()
    |> Enum.map(&calculate_extra_fuel_requirement_for_mass(&1))
    |> Enum.sum()
  end

  def calculate_extra_fuel_requirement_for_mass(mass) do
    fuel = calculate_fuel_requirement_for_mass(mass)
    calculate_extra_fuel_requirement_for_mass_acc(calculate_fuel_requirement_for_mass(fuel), fuel)
  end

  def calculate_extra_fuel_requirement_for_mass_acc(extra, total) when extra <= 0, do: total

  def calculate_extra_fuel_requirement_for_mass_acc(extra, total) do
    calculate_extra_fuel_requirement_for_mass_acc(calculate_fuel_requirement_for_mass(extra), total + extra)
  end

  defp calculate_fuel_requirement_for_mass(mass) when is_bitstring(mass) do
    mass
    # All of our current input data are specifically Integers.
    |> String.to_integer()
  ...
  defp process_input(input), do: String.split(input, ~r/\R/)
end
